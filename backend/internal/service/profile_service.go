...
func (s *ProfileService) GetProfileByUserID(userID uint) (*models.UserProfile, error) {
	profile, err := s.userRepo.GetUserProfileByUserID(userID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			// 如果 profile 不存在，可以選擇回傳一個空的 profile，或建立一個預設的
			log.Printf("Profile for user ID %d not found, creating a default one.", userID)
			...
			if createErr := s.userRepo.CreateUserProfile(defaultProfile); createErr != nil {
				log.Printf("Failed to create default profile for user ID %d: %v", userID, createErr)
				return nil, errors.New("profile not found and failed to create default")
			}
			return defaultProfile, nil
		}
		// 其他資料庫錯誤
		log.Printf("Error getting profile from repo for user ID %d: %v", userID, err)
		return nil, err
	}
	return profile, nil
}
...
// UpdateBio 更新個人簡介
func (s *ProfileService) UpdateBio(userID uint, bio string) (*models.UserProfile, error) {
	...
	if err := s.userRepo.UpdateUserProfile(profile); err != nil {
		log.Printf("Error updating bio in repo for user ID %d: %v", userID, err)
